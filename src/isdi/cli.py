...
@cli.command()
@click.option('--host', default=None, help='Host to bind to (default: 127.0.0.1 in debug, 0.0.0.0 in production)')
...
def run(host, port, debug, test_mode, no_browser):
    ...
    app.run(
        ...
        reloader_type='stat' if config.DEBUG else None,  # Use stat-based reloader for better reliability
        extra_files=None,
    )
...
def reset():
    ...
    for dir_path in [config.scans_dir, config.reports_dir, config.dumps_dir, config.phone_dumps_dir]:
        if dir_path.exists():
